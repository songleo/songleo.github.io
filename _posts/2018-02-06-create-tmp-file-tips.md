...
tempfile.NamedTemporaryFile会创建一个临时文件，并返回一个文件对象，通过该对象的name属性可以获取临时文件名，且创建的临时文件会在关闭后会自动删除。下面这段python代码创建一个临时文件，并再次打开该临时文件，写入数据，然后再次打开，读取文件，并按行打印出文件内容。
...
```
root@ssli_RHEL64:demo$ python tmp_file.py
