...
- 重复的 `og:type` 元数据，以及无法解析的 URL；错误会附带文件位置，不中断其他链接的扫描。
...
`srcset` 和图片预加载的 `imagesrcset` 按候选 URL 逐项检查，保留 URL 内部的逗号；首项为 `data:` 时仍检查后续候选项。URL 分词依据 [html 标准](https://html.spec.whatwg.org/multipage/images.html#parsing-a-srcset-attribute)，不验证宽度/密度描述符是否合法，也不模拟浏览器选择哪张图片。

## 2026-09-08 第二轮代码扫描

- 归档原先使用构建机器的年月，在 UTC 构建时会把月初的北京时间文章分到上月。317 个活跃 markdown 文件中有 7 个发布时间跨越这条月份边界。现按文章指定时区或站点时区分组，并复用文章列表的排序规则；没有修改文章发布时间或历史路由。
- 标签去重改用 map，保留同一 slug 首次出现的显示名称，排序明确使用站点语言，避免不同机器默认语言改变顺序。
- 返回地址读写集中处理：只接受同源 http/https 地址，保留分页、查询参数和锚点；会话存储不可用时，当前客户端导航仍保留最近列表地址。刷新页面且存储不可用时回退到按钮原有首页链接。
- 搜索清空操作不再读取按钮不存在的输入值，同时清理地址栏和返回地址中的 `q`，保留其他参数；页面离开后不再执行延迟初始化。搜索仍为关闭状态。
- `pnpm check:unit` 覆盖浏览器交互、时区分组、标签语义和返回地址处理；CI 和部署流程均执行。`pnpm check:browser` 保留为仅运行浏览器测试的入口。

本轮使用隔离的单元测试与临时 html fixture 验证修改，没有运行整站构建或线上浏览器验收；这份记录不代表当前部署已通过。
