...
通过使用time包中的Tick函数，可以实现一个简单的定时器，配合for循环可以周期性的执行某些操作。示例中可以看到，执行示例程序，每隔3s会打印当前的系统时间。
...
通过bytes.Buffer的缓存机制，在连接大量字符串时，可以大大的提升性能。由示例中可以看到，这里模拟了10万个字符串连接，使用传统的加号方式和使用bytes.Buffer方式，性能相差上万倍。其实不止go语言提供了这种缓存机制，大多数语言都有类似方式实现字符串连接。
